...
class SDKServer {
  ...
  async authenticateRequest(req: any): Promise<AuthenticatedUser> {
    ...
    // Reconcile the current account on every request. This preserves the stored
    // profile fields and lets an email allowlist promote an account that was
    // first created before it received administrator authorization.
    await db.upsertUser({
      openId: user.openId,
      name: user.name,
      email: user.email,
    ...
    if (!refreshedUser) {
      throw ForbiddenError("User not found after role reconciliation");
    }
